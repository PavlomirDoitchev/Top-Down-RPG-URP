...
public class ThirdPersonCamera : MonoBehaviour
{
    ...
    {
        var position = (focus != null ? focus.position : Vector3.zero) + focusOffset;
        var x = Mathf.SmoothDamp(focusPoint.x, position.x, ref currentXVelocity, horizontalSmoothTime);
        var y = Mathf.SmoothDamp(focusPoint.y, position.y, ref currentYVelocity, verticalSmoothTime);
        var z = Mathf.SmoothDamp(focusPoint.z, position.z, ref currentZVelocity, horizontalSmoothTime);
        focusPoint = new Vector3(x, y, z);
    }
